...
import { createSubagentRegistryReadContextFromRuns } from "./subagent-registry-read.js";
...
describe("subagent registry read context", () => {
  it("matches existing query helpers while reusing one indexed snapshot", () => {
    ...
    const context = createSubagentRegistryReadContextFromRuns({ runs });

    expect(context.listRunsForController(root)).toEqual(listRunsForControllerFromRuns(runs, root));
    expect(context.getSessionDisplayRunByChildSessionKey(parent)).toEqual(
      getSubagentRunByChildSessionKeyFromRuns(runs, parent),
    );
    expect(context.countActiveDescendantRuns(root)).toBe(
      countActiveDescendantRunsFromRuns(runs, root),
    );
    expect(context.countActiveDescendantRuns(root)).toBe(1);
  });
  ...
  it("handles empty registry snapshots", () => {
    ...
    expect(context.listRunsForController("agent:main:main")).toEqual([]);
    expect(context.getSessionDisplayRunByChildSessionKey("agent:main:subagent:missing")).toBeNull();
    expect(context.countActiveDescendantRuns("agent:main:main")).toBe(0);
  });
  ...
  it("uses requesterSessionKey when controllerSessionKey is missing", () => {
    ...
    expect(context.listRunsForController(root)).toEqual(listRunsForControllerFromRuns(runs, root));
    expect(context.listRunsForController(root)).toEqual([run]);
  });
  ...
  it("keeps moved middle descendants under the latest requester", () => {
    ...
    expect(context.countActiveDescendantRuns(root)).toBe(0);
    expect(context.countActiveDescendantRuns(otherRoot)).toBe(
      countActiveDescendantRunsFromRuns(runs, otherRoot),
    );
    expect(context.countActiveDescendantRuns(otherRoot)).toBe(2);
  });
  ...
  it("keeps one snapshot stable for the lifetime of the context", () => {
    ...
    expect(context.listRunsForController(root).map((run) => run.runId)).toEqual(["run-original"]);
    expect(
      createSubagentRegistryReadContextFromRuns({ runs })
        .listRunsForController(root)
        .map((run) => run.runId),
    ).toEqual(["run-original", "run-added-after-context"]);
  });

  it("matches existing display lookup behavior for whitespace-padded child session keys", () => {
    ...
    expect(context.getSessionDisplayRunByChildSessionKey(normalizedChildSessionKey)).toBe(
      getSubagentRunByChildSessionKeyFromRuns(runs, normalizedChildSessionKey),
    );
    expect(context.getSessionDisplayRunByChildSessionKey(normalizedChildSessionKey)).toBeNull();
  });
  ...
  it("keeps the display-row preference for in-memory records over persisted snapshots", () => {
    ...
    const context = createSubagentRegistryReadContextFromRuns({
      runs: persistedRuns,
      inMemoryRuns,
    });

    expect(context.getSessionDisplayRunByChildSessionKey(childSessionKey)?.runId).toBe(
      "run-memory-older-ended",
    );
  });
});
